...
public class IzingSimulation {
	...
	private static final long MAX_STEPS = MAX_ITERARIONS / ITERATIONS_PER_STEP;
	...
	public static void main(String[] args) {
		...
		final Simulator<IzingSystem<DescreteVector2D, SpinTable2D>, Integer, Checker<IzingSystem<DescreteVector2D, SpinTable2D>, Integer>> simulator = new Simulator<IzingSystem<DescreteVector2D, SpinTable2D>, Integer, Checker<IzingSystem<DescreteVector2D, SpinTable2D>, Integer>>(
				system, (int) ITERATIONS_PER_STEP, checker, s -> {
					if (steps < MAX_STEPS) {
						return true;
					} else {
					...
					result.append(PARAMETER_NAMES[0], new DoubleResult((double) (steps * ITERATIONS_PER_STEP)));
					result.append(PARAMETER_NAMES[1], new DoubleResult(TEMPERATURE));
					result.append(PARAMETER_NAMES[2], new DoubleResult(MAGNETIC_TENSION));
					result.append(PARAMETER_NAMES[3], new DoubleResult(state.getMagnetization()));
					result.append(PARAMETER_NAMES[4], new DoubleResult(state.getEnergy()));
					result.append(PARAMETER_NAMES[5], new DoubleResult((double) state.getLastAdanceFlips()));
				});
		AutopaintingField frame = new AutopaintingField(
				() -> transformToImage(simulator.getSimulation().getSystemSpinstate()), REPAINT_TIME);
	...
	private static Image transformToImage(SpinTable2D data) {
		...
		for (int i = 0; i < size.getX(); i++) {
			for (int j = 0; j < size.getY(); j++) {
				res.setRGB(i, j, data.get(i, j) ? SPIN_PLUS : SPIN_MINUS);
			}
		}
		return res;
	}
